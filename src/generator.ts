...
const defaultOptions: GeneratorOptions = {
    ...
    generateIndex: false,
    topLevelComment: "// auto-generated by wsdl-client",
};

function addTopLevelComment(sourceFile: SourceFile, options: Partial<GeneratorOptions>) {
    if (options.topLevelComment) {
        sourceFile.insertText(0, (writer) => {
            writer.writeLine("// auto-generated by wsdl-client");
            writer.blankLine();
        });
    }
}
...
function addSafeImport(
    ...
    if (!imports.find((imp) => imp.moduleSpecifier == moduleSpecifier)) {
        imports.push({
            moduleSpecifier,
            namedImports: [{ name: namedImport, alias }],
        });
    }
}
...
function generateDefinitionFile(
    ...
    addTopLevelComment(defFile, options);
...
export async function generate(
    ...
    for (const service of parsedWsdl.services) {
        ...
        for (const port of parsedWsdl.ports) {
                ...
                if (method.paramDefinition !== null) {
                    if (!allDefinitions.includes(method.paramDefinition)) {
                        ...
                        addSafeImport(
                            ...
                            method.paramDefinition.name,
                            `${method.paramDefinition.name}Def`
                        );
                    }
                    addSafeImport(
                        portImports,
                    ...
                    parameters: [
                        {
                            ...
                            type: method.paramDefinition ? `${method.paramDefinition.name}Def` : "{}",
                        },
                        {
                            name: "callback",
                            type: `(err: any, result: ${
            ...
            if (!mergedOptions.emitDefinitionsOnly) {
                addSafeImport(serviceImports, `../ports/${port.name}`, port.name, `${port.name}Port`);
                servicePorts.push({
                ...
                addTopLevelComment(portFile, options);
        ...
        if (!mergedOptions.emitDefinitionsOnly) {
            ...
            addTopLevelComment(serviceFile, options);
    ...
    if (!mergedOptions.emitDefinitionsOnly) {
        ...
        const createClientDeclaration = clientFile.addFunction({
            name: `create${camelcase(parsedWsdl.name, { pascalCase: true })}ClientAsync`,
        ...
        addTopLevelComment(clientFile, options);
